...
import { modifier_item_decay_tome } from "../modifiers/modifier_item_decay_tome";
import { modifier_item_decay_tome_debuff } from "../modifiers/modifier_item_decay_tome_debuff";

@registerAbility()
export class item_decay_tome extends BaseItem
{
    duration ?:number;
    ...
    OnSpellStart(): void {
        if (IsServer())
        {
            ...
            if (target)
            {
                target.AddNewModifier(this.GetCaster(), this, modifier_item_decay_tome_debuff.name, {duration : this.duration})
            }
        }
    }
